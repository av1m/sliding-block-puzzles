...
class PuzzleTestCase(unittest.TestCase):
    def test_index3(self):
        ...
        self.assertEqual((2, 1), puzzle.get_index(0))
        self.assertEqual((0, 0), puzzle.get_index(1))
        self.assertEqual((2, 2), puzzle.get_index(8))
        self.assertEqual((1, 1), puzzle.get_index(5))

    def test_index4(self):
        puzzle: Puzzle = Puzzle(
            [[4, 1, 2, 3], [5, 6, 7, 11], [8, 9, 10, 15], [12, 13, 14, 0]]
        )
        self.assertEqual((3, 3), puzzle.get_index(0))
        self.assertEqual((0, 1), puzzle.get_index(1))
        self.assertEqual((2, 0), puzzle.get_index(8))
        self.assertEqual((1, 0), puzzle.get_index(5))

    def test_swap3(self):
        puzzle: Puzzle = Puzzle([[1, 2, 3], [4, 5, 6], [7, 0, 8]])
        self.assertEqual(
            [[5, 2, 3], [4, 1, 6], [7, 0, 8]], puzzle._transition((0, 0), (1, 1))
        )
        self.assertEqual(
            [[1, 2, 3], [0, 5, 6], [7, 4, 8]], puzzle._transition((2, 1), (1, 0))
        )
    ...
    def test_possible_moves3(self):
        ...
        self.assertTrue(all(x in puzzle.get_possible_actions() for x in moves))
        ...
        self.assertTrue(
            all(
                x in Puzzle.puzzles_to_list(puzzle.get_possible_actions())
                for x in moves
            )
        )
